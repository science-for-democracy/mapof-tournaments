...
from mapel.elections.other.winners import (compute_borda_winners,
...
class Election(Instance):
  ...
  def compute_alternative_winners(self, method=None, party_id=None, num_winners=None):
    ...
    self.alternative_winners[party_id] = unmap_the_winners(winners_without_party_id,
                                                           party_id,
                                                           num_winners)

  def print_euclidean_voters_and_candidates_map(self,
                                                show=True,
                                                radius=None,
                                                name=None,
                                                alpha=0.5,
  ...
  # eps=1e-6,
    ...
    if algorithm == 'PCA':
      ...
      self.coordinates[object_type] = MDS_object.fit_transform(
  ...
  def get_feature(self, feature_id, feature_long_id=None, overwrite=False, **kwargs):
    if feature_long_id is None:
      feature_long_id = feature_id
