...
def generate_approval_vcr_votes(num_voters: int = None,
  ...
  for v in range(num_voters):
    for c in range(num_candidates):
      if v_range[v] + c_range[c] >= np.linalg.norm(voters[v] - candidates[c], ord=dim):
        votes[v].add(c)
...
def generate_2d_gaussian_party(num_voters=None, num_candidates=None, params=None):
  ...
  for j in range(params['num_parties']):
    for w in range(params['num_winners']):
      _id = j * params['num_winners'] + w
      candidates[_id] = [
          np.random.normal(params['party'][j][0][0], params['var']),
  ...
  for j in range(num_voters):
    voters[j] = [
        np.random.random() * (x_max - x_min) + x_min + shift[0],
...
def generate_ordinal_euclidean_votes(model: str = 'euclidean',
                                     num_voters: int = None,
                                     num_candidates: int = None,
                                     dim: int = 2,
                                     space: str = 'uniform',
