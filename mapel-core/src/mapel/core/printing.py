...
def _import_values_for_feature(experiment,
  ...
  if isinstance(feature_id, str):
    if feature_id in experiment.features:
      ...
      if column_id == 'time':
        values = {k: convert_none_time(v)
                  for k, v in values.items()}
      else:
        values = {k: convert_none(v)
                  for k, v in values.items()}
    else:
...
def _color_map_by_feature(experiment=None,
  ...
  for um in unique_markers:
    ...
    if um == '.':
      images.append(
          ax.scatter(xx[masks], yy[masks], c='grey', alpha=0.6, marker=um, s=mses[masks]))
        ...
        if family_id in {'A', 'B', 'C', 'WAL', 'CON', 'D', 'E'}:
          ...
          for i in range(MAL_COUNT):
            ...
            if family_id == 'A':  # Mal 0.
              ...
              color = 'black'
            ax.scatter(
                [experiment.coordinates_by_families[family_id][0][i] for _ in range(2)],
          ...
          if len(experiment.coordinates_by_families[family_id][0]) == 1:
            ax.scatter(
                [experiment.coordinates_by_families[family_id][0][0] for _ in range(2)],
...
def get_values_from_file_old(experiment,
                             experiment_id,
                             values,
                             normalizing_func=None,
                             marker_func=None):
