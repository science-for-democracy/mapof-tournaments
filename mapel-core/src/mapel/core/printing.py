...
def print_map_2d(experiment,
    ...
    def update_annot(inds, scs):
        ...
        for ind, sc in zip(inds, scs):
            text.append("\n".join([list(experiment.families.values())[sc].instance_ids[n] for n in ind]))
        text = "\n".join(text)
        print(text)
        annot.set_text(text)
...
def print_map_2d_colored_by_features(experiment,
    ...
    _color_map_by_features(experiment=experiment,
                           fig=fig,
                           ax=ax,
...
def print_map_3d(experiment,
    ...
    # COLORING
    if feature_id is not None:
        _color_map_by_feature(experiment=experiment,
                              fig=fig,
                              ax=ax,
                              feature_id=feature_id,
                              normalizing_func=normalizing_func,
    ...
    # BACKGROUND
    _basic_background(ax=ax, legend=legend, saveas=saveas, xlabel=xlabel, title=title, title_pos=title_pos)
...
def _import_values_for_feature(experiment,
    ...
    if isinstance(feature_id, str):
        if feature_id in experiment.features:
            ...
            values = imports.get_values_from_csv_file(experiment,
                                                      feature_id=feature_id,
            ...
            if feature_id == 'partylist' and column_id == 'value':
                bounds = imports.get_values_from_csv_file(experiment,
                                                          feature_id=feature_id,
                                                          feature_long_id=feature_id,
                                                          upper_limit=upper_limit,
                                                          lower_limit=lower_limit,
    ...
    _avg = sum(x for x in values.values() if x is not None) / len(values)
    ...
    return xx, yy, zz, shades, markers, mses, local_min, local_max, _avg, blank_xx, blank_yy, names
...
def _import_values_for_features(experiment,
    ...
    values_1 = imports.get_values_from_csv_file(experiment,
                                                feature_id=feature_ids[0],
                                                column_id=column_id,
                                                upper_limit=upper_limit)

    values_2 = imports.get_values_from_csv_file(experiment,
...
def _color_map_by_feature(experiment=None,
                          ...
                          strech=None,
                          colors=None):
    xx, yy, zz, shades, markers, mses, _min, _max, _avg, blank_xx, blank_yy, names = \
    ...
    for um in unique_markers:
        masks = (markers == um)
        if um == '.':
            images.append(ax.scatter(xx[masks], yy[masks], c='grey', alpha=0.6, marker=um, s=mses[masks]))
...
def _color_map_by_features(experiment=None,
                           fig=None,
                           ax=None,
                           feature_ids=None,
                           upper_limit=np.infty,
    ...
    if dim == 2:
        ...
        # VERTICAL VERSION
        cb_0 = fig.colorbar(images[0], orientation="vertical", pad=-0.1, shrink=0.55, ticks=ticks)
        ...
        cb_1 = fig.colorbar(images[1], orientation="vertical", pad=-0.1, shrink=0.55, ticks=ticks)
        # cb_1.ax.xaxis.set_ticks(ticks_pos)
        # cb_1.ax.set_xticklabels(xticklabels)
        cb_1.ax.set_yticklabels([])

        cb_2 = fig.colorbar(images[2], orientation="vertical", pad=0, shrink=0.55, ticks=ticks)
...
def add_advanced_points_to_picture_3d(fig,
                                      ax,
                                      experiment,
                                      experiment_id,
                                      values=None,
                                      ...
                                      ms=None,
                                      normalizing_func=None):
    xx, yy, zz, shades, markers, _min, _max = get_values_from_file_3d(experiment, experiment_id, values,
                                                                      normalizing_func)
    unique_markers = set(markers)
    images = []
...
def basic_coloring_with_shading(experiment=None, ax=None, urn_orangered=False):
    for family in experiment.families.values():
        if family.show:
            label = family.label
            for i in range(family.size):
                ...
                if 'Mallows (triangle)' in label:
                    ...
                    tint = 2 * tint
                    color = (0.75 - 0.75 * alpha + 0.125 * tint + 0.875 * alpha * tint, 0.75 - 0.75 * alpha,
...
def _add_textual(experiment=None,
                 textual=None,
                 ax=None,
                 size=16,
                 shades_dict=None,
    ...
    def my_text(x1, y1, text, color="black", alpha=1., size=size, b_color='black', boxstyle="round"):
        ax.text(x1,
                y1,
                text,
                size=size,
                ...
                if family_id in {'A', 'B', 'C', 'WAL', 'CON', 'D', 'E'}:
                    ...
                    for i in range(MAL_COUNT):
                        ...
                        if family_id == 'A':  # Mal 0.
                            ...
                            color = 'black'
                        ax.scatter([experiment.coordinates_by_families[family_id][0][i] for _ in range(2)],
                                   [experiment.coordinates_by_families[family_id][1][i] for _ in range(2)],
                                   color=color,
                                   alpha=1.,
                                   s=ms + 6,
                                   marker=experiment.families[family_id].marker)
                    ...
                    if len(experiment.coordinates_by_families[family_id][0]) == 1:
                        ax.scatter([experiment.coordinates_by_families[family_id][0][0] for _ in range(2)],
                                   [experiment.coordinates_by_families[family_id][1][0] for _ in range(2)],
...
def get_values_from_file_old(experiment, experiment_id, values, normalizing_func=None, marker_func=None):
    path = os.path.join(os.getcwd(), "election", experiment_id, "features", str(values) + ".txt")
