...
def wl1(vector_1: np.ndarray, vector_2: np.ndarray) -> float:
  """ Return: L1 distance """
  return sum(
      [len(vector_1) - i * abs(vector_1[i] - vector_2[i]) for i in range(len(vector_1))])
...
def hellinger(vector_1, vector_2):
  ...
  return math.sqrt(1 - (1 / math.sqrt(h1 * h2 * len(vector_1) * len(vector_1))) * product)
...
def emd2(vector_1, vector_2):
  """ compute EMD metric """
  vector_1 = deepcopy(vector_1)
  dirt = 0.
  for i in range(len(vector_1) - 1):
    surplus = vector_1[i] - vector_2[i]
    dirt += (surplus * surplus)
    vector_1[i + 1] += surplus
  return dirt
