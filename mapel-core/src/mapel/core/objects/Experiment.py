...
class Experiment:
    ...
    def compute_coordinates_by_families(self, dim=2) -> None:
        ...
        if self.families is None:
            ...
            for i, instance_id in enumerate(self.instances):
                ...
                self.families[instance_id] = Family(culture_id=model,
                                                    family_id=family_id,
                                                    label=label,
                                                    alpha=alpha,
                                                    color=color)
            ...
            for family_id in self.families:
                ...
                if dim >= 3:
                    for instance_id in self.families[family_id].instance_ids:
                        coordinates_by_families[family_id][2].append(self.coordinates[instance_id][2])
    ...
    def import_feature(self, feature_id, column_id='value', rule=None):
        ...
        return imports.get_values_from_csv_file(self,
    ...
    def print_correlation_between_distances(self,
        ...
        def nice(name):
            return {
                ...
                'degree_cen': 'Degree Centrality',
                'eigen_cen': 'Eigenvector Centrality',
                'katz_cen': 'Katz Distance',
                'closeness_cen': 'Closeness Centrality',
                'betweenness_cen': 'Betweenness Centrality',
                'load_cen': 'Load Centrality',
                'harmonic_cen': 'Harmonic Centrality',
                'laplacian_cen': 'Laplacian Centrality',
                'pagerank': 'PageRank'
            }.get(name, name)
    ...
    def merge_election_images(self, size=250, name=None, show=False, ncol=1, nrow=1, object_type=None):
    ...
    def merge_election_images_in_parts(self,
                                       size=250,
                                       name=None,
                                       show=False,
                                       ncol=1,
    ...
    def merge_election_images_double(self, size=250, name=None, distance_ids=None, show=False, ncol=1, nrow=1):
        images = []
        for i, election in enumerate(self.instances.values()):
