...
from mapel.core.embedding.simulated_annealing.simulated_annealing import \
    SimulatedAnnealing
...
def embed(experiment_id,
            ...
            if i < j:
                ...
                if embedding_id in {'fr', 'spring'}:
                    ...
                    if num_neighbors is not None:
                        ...
                        sorted_list_1 = list((dict(sorted(tmp.items(), key=lambda item: item[1]))).keys())
                        tmp = experiment_id.distances[instance_id_2]
                        sorted_list_2 = list((dict(sorted(tmp.items(), key=lambda item: item[1]))).keys())
    ...
    if embedding_id.lower() in {'fr', 'spring'}:
        ...
        my_pos = TSNE(n_components=dim, n_iter=num_iterations, **kwargs).fit_transform(x)
        ...
        my_pos = Isomap(n_components=dim, n_neighbors=num_neighbors, **kwargs).fit_transform(x)
